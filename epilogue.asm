...
L_code_ptr_bin_apply:
    ...
    mov rax, qword [rbp + 2 * 8]     ; rax := argument count
    cmp rax, 2                       ; Check if there are at least 2 arguments
    jb L_error_arg_count_2           ; Error if fewer than 2 arguments

    ; Retrieve the function (first argument) and the list (second argument)
    mov rbx, qword [rbp + 3 * 8]     ; rbx := function (closure object)
    mov rcx, qword [rbp + 4 * 8]     ; rcx := list (second argument)

    ; Initialize argument splicing
    xor rsi, rsi                     ; rsi := total expanded argument count
    lea rdi, [rbp - 8]               ; rdi := temporary stack pointer for arguments

splice_arguments:
    ; Check if the list is empty
    cmp rcx, sob_nil         ; Is the list empty?
    je apply_done_splicing           ; If yes, we’re done

    ; Extract the car (first element of the list) and cdr (rest of the list)
    mov rdx, qword [rcx + 1]         ; rdx := car of the list
    mov rcx, qword [rcx + 9]         ; rcx := cdr of the list

    ; Push the extracted argument onto the temporary stack
    mov qword [rdi + rsi * 8], rdx   ; Store rdx in temporary stack
    inc rsi                          ; Increment argument count
    jmp splice_arguments             ; Continue splicing

apply_done_splicing:
    ; Push spliced arguments onto the real stack
    mov rdx, rsi                     ; rdx := spliced argument count
    test rdx, rdx                    ; Check if any arguments were spliced
    je no_arguments_to_push          ; If none, skip pushing arguments

push_arguments_loop:
    dec rdx                          ; rdx := rdx - 1
    mov r8, qword [rdi + rdx * 8]    ; Load argument from temporary stack
    push r8                          ; Push argument onto real stack
    test rdx, rdx                    ; Check if more arguments remain
    jne push_arguments_loop          ; If yes, continue pushing

no_arguments_to_push:
    ; Adjust total argument count
    add rsi, 1                       ; rsi := rsi + 1 (for the function itself)

    ; Call the function
    mov rdi, rbx                     ; rdi := closure (function)
    call L_code_ptr_apply_closure    ; Call the function

    ; Clean up the stack after the call
    add rsp, rsi * 8                 ; Pop spliced arguments from stack
    leave
    ret
